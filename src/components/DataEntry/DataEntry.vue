      ...
      <div class="search">
        <div class="up">
          ...
          <el-input
            ...
            @keyup.enter.native="getTable()"
          ></el-input>
          <el-select v-model="entry_status" size="mini" style="width:180px;margin-left:5px" @change="getTable()" 
           clearable filterable placeholder="请选择校验状态">
            <el-option v-for="(item,index) in  this.entry_statusList" :key="index" :value="item">
              <span>{{item}}</span>
            </el-option>
          </el-select>
...
export default { 
  ...
  methods: {
    ...
    async chakan(row) {
      ...
      // console.log(res);
      // 将获取到的IDarray存下来
      res.id_array.map( (item,index) =>{
        this.id_array.push(parseInt(item.id))
      })
      console.log('点击数据集查看获取的数组',this.id_array)
      // this.id = row.id;
    ...
    async fou(id) {
      ...
      const { data: res } = await this.axios.get(
        "excel_data/onedata.php?id=" + this.id
      );
      if (res.ok == 0) {
    ...
    del(row) {
      this.$confirm("确定删除该数据？是否继续?", "提示", {
        ...
        .then(() => {
          ...
          // 删除数据后更新id_array
          for(var i = 0;i < this.id_array.length;i++){
            if(row.id == this.id_array[i]){
              console.log('当前下标',i)
              this.id_array.splice(i,1)
              console.log('删除数据后更新数组',this.id_array)
            }
          }
